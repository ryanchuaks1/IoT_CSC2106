...
def count_vehicles(source, model):
    camera = Picamera2()
    print("launching...")
    config = camera.create_still_configuration(main={"size": (4056,3040)}, lores = {"size": (480,320)}, display = "lores", buffer_count=3, queue=False)
    camera.configure(config)
    camera.set_controls({"ExposureTime":10000, "AnalogueGain": 5})
    camera.start(show_preview=False)
    print("camera finished configuring")
